...
def build_chart_data(request: ChartRequest) -> ChartResponse:
    try:
        ...
        if not candles:
            logger.warning(f"Свечи не получены для {request.ticker}/{request.timeframe}")
        ...
        logger.exception("Ошибка при подготовке данных графика")
        raise RuntimeError("Внутренняя ошибка при подготовке данных графика") from exc
